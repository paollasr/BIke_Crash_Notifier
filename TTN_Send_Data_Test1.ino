...
HCSR04 hc(7, 8); //trigger and echo
bool range = false;
...
uint8_t senseDistance(void) {
  uint16_t distancia = hc.dist();
#ifdef DEBUG
  Serial.print(F("Distance measured: "));
  Serial.print(distancia);
#endif

  if ( distancia < 2 || distancia > 2000 )    // This is an error
    distancia = 0;                            // Use 0 to indicate an error
  else if ( distancia > 255 )
    distancia = 255;                          // All bigger than 255 = 255 to fit within a byte
...
uint16_t senseFalls(void) {
  // TBD needs work....
  if ( /* real sensor flag set or so?? Simulate a 25% fall every once in a while for now */ millis() % 100 < 25 )  {
    freeFallDetectCount++;
...
/*
   Check if the rider is present
*/
uint32_t riderLastSeen = 0;
const int riderBounceDelay = 30 * 1000;     // Rider has to be undetected for this many milliseconds before we believe (s)he has gone.
bool isRiderPresent(void) {
  // TBD needs work....
  if ( /* real sensor flag set or so?? Simulate a 50% present */ micros() % 100 < 20 )  {
    riderLastSeen = millis();
    return true;
  }
  if ( millis() - riderLastSeen > riderBounceDelay )
    return false;
  else
    return true;
}
...
#ifdef DEBUG
    Serial.println("Rider present; let's rock.");
#endif
